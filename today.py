        ...
        if repo_hash == hashlib.sha256(edges[index]['node']['nameWithOwner'].encode('utf-8')).hexdigest():
            try:
                if int(commit_count) != edges[index]['node']['defaultBranchRef']['target']['history']['totalCount']:
                    # if commit count has changed, update commit count in cache
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data):
    ...
    justify_format(root, 'star_data', star_data, 23)
...
if __name__ == '__main__':
    ...
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2004, 2, 27))
